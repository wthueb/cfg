...
toggle_slider() {
    if [[ $(sketchybar --query volume_icon | jq -r ".popup.drawing") == "on" ]]; then
        toggle_devices
    fi
...
toggle_devices() {
    ...
    while IFS= read -r device; do
        ...
        args+=(--add item volume.device.$COUNTER popup."$NAME" \
            ...
            label.color="$COLOR" \
            click_script="SwitchAudioSource -s \"${device}\" && sketchybar --set /volume.device\.*/ label.color=$WHITE --set \$NAME label.color=$WHITE --set $NAME popup.drawing=off")
